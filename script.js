...
const CAT_CHAT_MESSAGES = {
  ...
  catFriendRight: {
    title: "呆呆猫小助理",
    text:
      "是忘了吧，我可不是AI助理哦～想我的话，可以给我打电话，约我吃饭，西湖边吹吹风，数星星～\n因为“人与人之间真实的链接，才是AI替代不了的东西”\n联系方式：lzyliangzi@gmail.com"
  }
};
...
const DEFAULT_TAG_CARD_MAP = {
  ...
  自由追光者: {
    ...
    image: "./assets/images/present/present-03.jpg"
  },
  "清醒小甜妹": {
    title: "清醒小甜妹",
    description: "甜是表达方式，清醒是底层逻辑。既能温柔，也能果断。",
    image: "./assets/images/present/present-08.jpg"
  },
  "请我喝奶茶吗": {
    title: "请我喝奶茶吗",
...
function showToast(text, duration) {
  ...
  const autoDuration = duration || Math.min(12000, Math.max(2600, Math.ceil(text.length / 18) * 900));
...
async function initHeroCarousel() {
  ...
  slides.forEach((item) => loadedMap.set(item.src, true));
...
function renderPresentCarouselRows() {
  ...
  const sources = Array.from({ length: 8 }, (_, idx) => {
...
async function playMusic(silent = false) {
  ...
  try {
    ...
    if (!silent) showToast("浏览器限制了自动播放，点一下 BGM 即可播放");
    return false;
  }
}
...
async function switchTrack() {
  if (!bgm || !trackSelect || !musicBtn) return;
...
function setupAutoPlay() {
  ...
  window.setTimeout(async () => {
    const ok = await playMusic(true);
    if (!ok) showToast("浏览器可能拦截了自动播放，点下 BGM 就能播放");
    if (ok) teardown();
  }, 160);
}
